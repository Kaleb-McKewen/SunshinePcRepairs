...
<script>
    ...
    const tagsImport = "<?php echo $tags; ?>";

    function createButton(tagContent) {
        //add to array
    ...
    //create buttons from existing data
    console.log(tagsImport);
    if (tagsImport != '[]' && tagsImport != "") {
        const split = tagsImport.split(",");
        split.forEach(name => {
            createButton(name);
        });
